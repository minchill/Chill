...
async def inv_cmd(ctx):
    ...
    for shop_id, count in custom_counts.items():
        ...
        item_name = item_data.get("name", "Custom Item")
...
async def profile_cmd(ctx, target: str = None):
  ...
  # ====================================================================
# PHẦN 8: LỆNH ADMIN VÀ UTILITY
# ====================================================================

@bot.command(name="setmoney", aliases=["Setmoney"])
async def set_money_cmd(ctx, user_id: str = None, amount: int = None):
    if not is_admin(ctx.author.id):
        return await ctx.send("❌ Lệnh này chỉ dành cho Admin.")
        
    if user_id is None or amount is None:
        return await ctx.send("❌ Cú pháp: `bsetmoney <user_id> <số_tiền>`")
        
    try:
        member_id = int(user_id.strip('<@!>'))
        update_balance(member_id, amount - get_user(member_id).get("balance", 0))
        member = await ctx.guild.fetch_member(member_id)
        
        await ctx.send(f"✅ Đã đặt số dư của **{member.display_name}** thành **{amount:,} xu**.")
    except Exception as e:
        await ctx.send(f"❌ Lỗi: Không thể tìm thấy user ID hoặc số tiền không hợp lệ. {e}")


@bot.command(name="addadmin", aliases=["Addadmin"])
async def add_admin_cmd(ctx, user_id: str = None):
    """Thêm người dùng vào danh sách Admin. (Khuyến nghị quản lý qua Railway)"""
    if not is_admin(ctx.author.id):
        return await ctx.send("❌ Lệnh này chỉ dành cho Admin.")
        
    if os.getenv("ADMIN_IDS"):
        return await ctx.send("⚠️ Danh sách Admin hiện đang được quản lý qua biến môi trường **ADMIN_IDS** trên Railway. Vui lòng cập nhật trực tiếp tại đó.")
    
    if user_id is None:
        return await ctx.send("❌ Cú pháp: `baddadmin <user_id>`")
        
    try:
        member_id = str(int(user_id.strip('<@!>')))
        if member_id not in global_data["admin_list"]:
            global_data["admin_list"].append(member_id)
            save_data(global_data)
            await ctx.send(f"✅ Đã thêm **{member_id}** vào danh sách Admin (local).")
        else:
            await ctx.send("❌ Người dùng này đã là Admin.")
    except:
        await ctx.send("❌ ID người dùng không hợp lệ.")
        
        
@bot.command(name="deladmin", aliases=["removeadmin", "Deladmin", "Removeadmin"])
async def del_admin_cmd(ctx, user_id: str = None):
    """Gỡ người dùng khỏi danh sách Admin. (Khuyến nghị quản lý qua Railway)"""
    if not is_admin(ctx.author.id):
        return await ctx.send("❌ Lệnh này chỉ dành cho Admin.")
    ...
    if user_id is None:
        return await ctx.send("❌ Cú pháp: `bdeladmin <user_id>`")

    try:
        member_id = str(int(user_id.strip('<@!>')))
        if member_id in global_data["admin_list"]:
            global_data["admin_list"].remove(member_id)
            save_data(global_data)
            await ctx.send(f"✅ Đã gỡ **{member_id}** khỏi danh sách Admin (local).")
        else:
            await ctx.send("❌ Người dùng này không phải là Admin.")
    except:
        await ctx.send("❌ ID người dùng không hợp lệ.")


@bot.command(name="help", aliases=["commands", "hlep", "Help", "Commands"])
async def help_cmd(ctx):
    # Logic cũ (cần tùy chỉnh thêm nếu có thời gian)
    embed = discord.Embed(
        title="📚 Hướng Dẫn Sử Dụng Bot Anime RPG",
        description="Prefix: `b`",
        color=discord.Color.yellow()
    )
    embed.add_field(name="💰 Kinh Tế & Shop", value="`bal`, `daily`, `shop`, `bpetshop`, `bequipshop`, `bbuy <ID>`, `bsell <ID>`", inline=False)
    embed.add_field(name="🎒 Túi Đồ & Item", value="`inv`, `buse <ID> [Pet_ID]`, `bequip <ID_6_số> <Pet_ID>`, `bunequip <Pet_ID>`", inline=False)
    embed.add_field(name="🐾 Pet & Hunt", value="`brollpet`, `bpets`, `bhunt`", inline=False)
    embed.add_field(name="💖 Tương Tác & Hôn nhân", value="`hug`, `kiss`, `pat`, `hit`... (@user), `propose`, `accept`, `divorce`, `wife`", inline=False)
    embed.add_field(name="🎨 Tùy Chỉnh & Profile", value="`bprofile`, `bsetbg bg <URL>`", inline=False)
    embed.add_field(name="🛠️ Admin (Dành cho Admin)", value="`bsetmoney <ID> <số tiền>`", inline=False)
    await ctx.send(embed=embed)


@bot.command(name="tts", aliases=["say", "bs", "Tts", "Say"])
async def tts_cmd(ctx, *, text):
    # Logic cũ
    try:
        ...
        if ctx.author.voice and ctx.author.voice.channel:
            ...
            vc.play(discord.FFmpegPCMAudio(filename), after=lambda e: print('Done playing', e))
            while vc.is_playing():
